...
import {
  getAnnualReportById,
  updateAnnualReport,
} from "../../../services/StorageManagement/AnnualReportApi";
...
export default function UpdateAnnualReport() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchAnnualReport = async () => {
      try {
        const response = await getAnnualReportById(id);
        const reportDate = response.data;

        // Format the date for the input field
        const formattedDate = reportDate.year
          ? new Date(reportDate.year).toISOString().split("T")[0]
          : "";

        setFormData({
          bookNumber: reportDate.bookNumber || "",
          province: reportDate.province || "",
          district: reportDate.district || "",
          year: formattedDate,
          waseqaType: reportDate.waseqaType || "",
          summaryOfWaseqa: reportDate.summaryOfWaseqa || "",
          remarks: reportDate.remarks || "",
        });
        setAvailableDistricts(
          provincesWithDistricts[reportDate.province] || []
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateAnnualReport(id, formData);
