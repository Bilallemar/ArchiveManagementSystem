...
import { updateAnnualReport } from "../../../services/StorageManagement/MakzanAnnualReportAPI";
import api from "../../../services/api";

export default function EditMakzanAnnualReportDialog({
  open,
  onClose,
  report,
  onSuccess,
  ...
  const handleLoadFromScanner = async () => {
    if (detectedFiles.length === 0) {
      toast.error(texts.noFilesToLoad || "د لوستلو لپاره فایل نشته");
      return;
    }
    try {
      ...
      setDetectedFiles([]);
      toast.success(
        `${files.length} ${texts.filesLoaded || "فایلونه لوډ شول"}`,
      );
    } catch (error) {
      toast.error(texts.filesLoadError || "د فایلونو لوډولو کې ستونزه");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateAnnualReport(report.id, formDataToSend);
