...
export default function AddMakzanSubmissionReport() {
  ...
  const handleLoadFromScanner = async () => {
    ...
    try {
      ...
      setDetectedFiles([]);
      toast.success(`${files.length} files loaded`);
    } catch (error) {
      console.error("Failed to load files from scanner", error);
      toast.error(texts.filesLoadError);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createMakzanSubmissionReport(formDataToSend);
    ...
    <Box sx={{ p: 3, maxWidth: 1400, mx: "auto" }}>
      ...
      <Box sx={{ mb: 4, display: "flex", alignItems: "center", gap: 2 }}>
        <Button
          ...
          onClick={() => navigate("/annual-reports-info")}
