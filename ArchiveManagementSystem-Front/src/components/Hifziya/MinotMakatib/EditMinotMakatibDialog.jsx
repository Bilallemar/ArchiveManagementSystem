...
export default function EditMinotMakatibDialog({ open, onClose, minotMakatib, onSuccess }) {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      toast[res.data.length ? "success" : "info"](`${res.data.length} فایلونه وموندل شول`);
    } catch {
  ...
  const handleRemoveExistingFile = (fileId) => {
    ...
    toast.success("فایل له لست څخه لیرې شو");
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("ریکارډ په بریالیتوب تازه شو");
      onSuccess();
      onClose();
    } catch (err) {
      toast.error("تازه کولو کې ستونزه");
    } finally {
    ...
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
      <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", pb: 1 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
          <Typography variant="h5" fontWeight="bold">د مینوټ مکاتب سمول</Typography>
                ...
                <Stack spacing={3}>
                  <Card variant="outlined">
                    <CardContent sx={{ textAlign: "center", py: 3 }}>
                      ...
                      <Typography variant="h6" gutterBottom>د سکینر حالت</Typography>
                      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                        {detectedFiles.length > 0 ? `${detectedFiles.length} فایلونه` : "د سکین لپاره کلیک وکړئ"}
                      </Typography>
                      <Button variant="outlined" fullWidth startIcon={<ScannerIcon />} onClick={handleScan} disabled={isScanning} sx={{ mb: 1 }}>
                        {isScanning ? <><CircularProgress size={16} sx={{ mr: 1 }} />سکین کیږي...</> : "سکین کړئ"}
                      </Button>
                      <Button
                  ...
                  {existingFiles.length > 0 && (
                    <Box>
                      <Typography variant="subtitle2" color="text.secondary" gutterBottom>
                        موجود فایلونه ({existingFiles.length})
                      </Typography>
                  ...
                  {formData.newFiles.length > 0 && (
                    <Box>
                      <Typography variant="subtitle2" gutterBottom>
                        نوي فایلونه ({formData.newFiles.length})
                      </Typography>
                      <Stack spacing={0.8} sx={{ maxHeight: 180, overflowY: "auto" }}>
                  ...
                  {existingFiles.length === 0 && formData.newFiles.length === 0 && (
                    <Alert severity="info" icon={<FolderIcon />}>تر اوسه هیڅ فایل ضمیمه شوی نه دی</Alert>
                  )}
                </Stack>
              </Grid>
              ...
              <Grid item xs={12} md={8}>
                <Grid container spacing={2.5}>
                  <Grid item xs={12} sm={6}>
                    <TextField fullWidth size="small" required name="cartonNumber" label="د کارتن شمېره"
                      value={formData.cartonNumber} onChange={handleInputChange} error={!formData.cartonNumber.trim()} />
                  </Grid>

                  <Grid item xs={12} sm={6}>
                    <TextField fullWidth size="small" name="letterNumber" label="د مکتوب شمېره"
                      value={formData.letterNumber} onChange={handleInputChange} />
                  </Grid>

                  <Grid item xs={12} sm={6}>
                    <TextField fullWidth size="small" name="year" label="کال" type="number"
                  ...
                  <Grid item xs={12}>
                    <TextField fullWidth size="small" name="subject" label="موضوع"
                  ...
                  <Grid item xs={12}>
                    <TextField fullWidth size="small" multiline rows={4} name="description" label="ملاحظات"
      ...
      <DialogActions sx={{ px: 3, py: 2 }}>
        <Button onClick={onClose} variant="outlined" disabled={isSubmitting}>لغوه</Button>
        <Button
          onClick={handleSubmit} variant="contained" disabled={isSubmitting}
          startIcon={isSubmitting ? <CircularProgress size={20} /> : <SaveIcon />}
          sx={{ bgcolor: "#2196F3", "&:hover": { bgcolor: "#1976D2" } }}
        >
          {isSubmitting ? "ذخیره کیږي..." : "تازه کړئ"}
        </Button>
      </DialogActions>
    </Dialog>
  );
