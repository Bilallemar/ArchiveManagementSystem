...
export default function EditHifziyaWaradaSaderaDialog({
  ...
  const direction = waradaSadara?.direction || "INCOMING";
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const [orgsRes, docTypesRes, scannerPathRes, recordRes] =
          await Promise.all([
            ...
            api
              ...
              .catch(() => ({ data: { path: "" } })),
            // We already have waradaSadara prop, but if needed you can re-fetch
          ]);
        ...
        if (waradaSadara) {
          ...
          setFormData({
            no: waradaSadara.no || "",
            receiverOrg: waradaSadara.receiverOrg?.id || "",
            senderOrg: waradaSadara.senderOrg?.id || "",
            ...
            senderOrgDate: convertGregorianToHijri(waradaSadara.senderOrgDate),
          ...
          if (waradaSadara.cabinetFile) {
            const cf = waradaSadara.cabinetFile;
    ...
    loadData();
  }, [open, waradaSadara, text.loadError]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        outgoingDate: convertHijriToGregorian(formData.outgoingDate) || null,
      ...
      await updateHifziyaWaradaSadera(waradaSadara.id, formDataToSend);
                ...
                <Grid container spacing={2.5}>
                  ...
                  <Grid item xs={12} md={6}>
                    <HijriDatePicker
                      ...
                      type="date"
                      name="senderOrgDate"
                      label={text.senderOrgDate || " د لېږونکي ادارې تاریخ"}
                      InputLabelProps={{ shrink: true }}
