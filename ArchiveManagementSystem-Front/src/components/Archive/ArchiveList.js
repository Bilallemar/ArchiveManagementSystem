...
import { gitAllArchives } from "../../services/ArchiveManagement/ArchiveAPI";
...
const columns = [
  { id: "docNo", label: "نمبر سند", minWidth: 100 },
  ...
  { id: "submitedDate", label: " تاریخ ارسال", minWidth: 150 },
  { id: "year", label: "سال", minWidth: 120 },
  { id: "docType", label: " نوعیت سند", minWidth: 100 },
  { id: "description", label: "ملاحظات", minWidth: 120 },
...
export default function AnnualReportList() {
  ...
  const handleEdit = () => {
    navigate(`/archives/${selectedArchive.id}`);
    handleClose();
  };
  ...
  const handleNewReport = () => {
    navigate("/archives/add-archive");
  };

  const handleDelete = async () => {
    try {
      await deleteHifziyaHazari(selectedArchive.id);
      loadArchive();
                  ...
                  .map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="center">
                          {new Date(row.incommingDate).toLocaleDateString("en-GB")}
                        </TableCell>
                         <TableCell align="center">
                          {new Date(row.outgoingDate).toLocaleDateString("en-GB")}
                        </TableCell>
                          <TableCell align="center">
                          {row.org?.name || "N/A"}
                        </TableCell>
                        <TableCell align="center">
                          {new Date(row.submitedDate).toLocaleDateString("en-GB")}
                        </TableCell>
