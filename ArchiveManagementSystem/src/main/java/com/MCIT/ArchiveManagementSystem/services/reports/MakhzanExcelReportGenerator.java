...
@Component
public class MakhzanExcelReportGenerator {
   ...
   * (لنډیز راپور). The raw tabular data (MakhzanReportDataDTO) is accepted
   * for backward compatibility with callers but is no longer rendered.
   */
  public byte[] generate(MakhzanReportDataDTO data, NarrativeReportDTO narrative) {
  ...
  private void writeSummarySheet(XSSFWorkbook workbook, NarrativeReportDTO narrative) {
    ...
    rowIdx = writeWrappedParagraph(sheet, rowIdx, "نتیجه‌گیری:", headingStyle);
  ...
  private int writeWrappedParagraph(XSSFSheet sheet, int rowIdx, String text, CellStyle style) {
    ...
    int lines = Math.max(1, (int) Math.ceil(safeText.length() / (double) APPROX_CHARS_PER_LINE));
